...
Function New-ModuleTemplate {
    ...
    # Create a folder with the same name as the module
    New-Item -Path $ModulePath -ItemType Directory
    Set-Location $ModulePath
    # Create the public folder to store your ps1 scripts
    New-Item -Path .\Public -ItemType Directory

    $ManifestParameters = @{
        ModuleVersion     = $ModuleVersion
        Author            = $Author
        # Set the path to the psd1 file
        Path              = ".\$($ModuleName).psd1"
        # Set the path to the psm1 file
        RootModule        = ".\$($ModuleName).psm1"
        PowerShellVersion = $PSVersion
    }
    # Create the module manifest psd1 file with the settings supplied in the parameters
