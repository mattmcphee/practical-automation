...
Function Add-SecondVHD{
    ...
    # Set the path for the second hard drive
    $Path = @{
        Path      = $VM.Path
    ...
    # If the VHD does not exist, create it
    if (-not(Test-Path $DataDisk)) {
        New-VHD -Path $DataDisk -SizeBytes 10GB | Out-Null
    }

    # If the VHD is not attached to the VM, attach it
    $Vhd = $VM.HardDrives | 
...
# Run the command on the guest OS to set up the new drive
$Command = @{
    VMId        = $VM.Id
